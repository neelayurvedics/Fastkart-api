...
class SettingRepository extends BaseRepository
{
    ...
    public function frontSettings()
    {
        try {
            ...
            // Cache the computed front settings for 5 minutes to improve response time
            return Cache::remember('front_settings', now()->addMinutes(5), function () {
    ...
    public function update($request, $id)
    {
        ...
        try {
            ...
            $this->setSystemReservedLanguage($settings?->values);

            // Clear cached front settings so clients get updated values
    ...
    public function setSystemReservedLanguage($settings)
    {
        $language = $this->language->findOrFail($settings['general']['default_language_id']);
        $this->language?->query()->update([ 'system_reserve' =>  false]);
        $language->update([
            'system_reserve' =>  true
        ]);
    }
        ...
        try {
            ...
            if (isset($value['general'])) {
                DotenvEditor::setKeys([
                    ...
                    'GOOGLE_MAP_API_KEY' => $value['general']["google_map_api_key"] ?? '',
                ]);
            }
